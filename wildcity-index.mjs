#!/usr/bin/env node
/**
 * WildCity Index — Índice de Biodiversidad Municipal
 * (antes: Wild Territory Index / WildCity Index)
 *
 * Calcula un score 0-100 para cualquier municipio o territorio:
 *   Pilar 1 (40%): Biodiversidad y Ecosistemas (EII, especies, parches)
 *   Pilar 2 (35%): Servicios y Oportunidades (carbono, créditos, conectividad)
 *   Pilar 3 (25%): Gobernanza y Riesgo (incendio, luz, protección, inversión)
 *
 * Uso:
 *   node wildcity-index.mjs "Somiedo"
 *   node wildcity-index.mjs "Somiedo" --full
 *   node wildcity-index.mjs "Somiedo" --investment 150000
 */
...
// Auto-detect Python environment (portable — no hardcoded paths)
const GEE_VENV = process.env.GEE_VENV || process.env.VIRTUAL_ENV || '';
const PYTHON = GEE_VENV ? path.join(GEE_VENV, 'bin', 'python') : 'python3';
...
// ─── SAFE PYTHON EXECUTION (no shell injection) ─────────────────────────
function runPython(scriptPath, args = [], timeoutMs = 60000) {
  // execFileSync does NOT use a shell → no injection possible
...
function getMunicipalityGeometry(name) {
  ...
  return runPython(script, [name], 30000);
}
...
function getGEEIndicators(geojsonPath) {
  ...
  return runPython(script, ['--geojson-file', geojsonPath], 600000);
}
...
async function main() {
  ...
  if (!args.length) {
    console.log('Uso: node wildcity-calculator.mjs "Municipio" [--full] [--investment EUR]');
    process.exit(1);
  }
...
╚══════════════════════════════════════════════════════════════════════════╝`);
  ...
  if (runFull) {
    ...
    try {
      ...
      const faOut = path.join(__dirname, `wti-full-${safeName}`);
      fullAnalysis = runPython(faScript, [
        '--lat', String(lat), '--lng', String(lng),
        '--buffer', '8000', '--name', safeName,
        '--output-dir', faOut, '--json'
      ], 1800000);
  ...
  // ═══ STEP 5: SCORING ═══
  log(`WildCity Index — SCORING — ${typology}`);
...
╚══════════════════════════════════════════════════════════════════════════╝`);
  ...
  const safeName = name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
  const safeName2 = name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
  const outPath = path.join(__dirname, `wildcity-${safeName2}.json`);
